...
function getAllWineries($jsonData, $DBConnection)
{
    ...
    if (isset($jsonData->searchWineries)) {
        ...
        foreach ($jsonData->searchWineries as $key => $value) {
            ...
            } else if ($key == "Wine_Tasting") {
                // $types .= "i";
                // $DBQuery .= "Wine_Tasting = ? AND ";
                // $params[] = $jsonData->searchWineries->Wine_Tasting;
                $DBQuery .= "Wine_Tasting = " . $jsonData->searchWineries->Wine_Tasting . " AND ";
            } else {
                $types .= "s";
    ...
    $result = mysqli_stmt_get_result($statement);
    $output = mysqli_fetch_all($result, MYSQLI_ASSOC);
    $returnJson = array("status" => "success", "timestamp" => time(), "data" => $output, "query" => $DBQuery, "Test" => "hazaaaaaaaa");
    // $returnJson = array("status" => "success", "timestamp" => time(), "data" => "Testing");
