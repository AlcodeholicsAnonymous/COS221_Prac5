...
function updateWine($jsonData, $DBConnection, $wineryID)
{
    ...
    foreach ($jsonData->parameters as $key => $value) {
        $DBQuery .= $key . " = ?, ";
        $params[] = array('value' => $value, 'type' => 's');
    }

    $DBQuery = substr($DBQuery, 0, -2);
    $DBQuery .= " WHERE Winery_ID = ? AND Name = ? AND Year = ? AND Type = ?";
    $params[] = array('value' => $wineryID, 'type' => 's');
    $params[] = array('value' => $jsonData->wineName, 'type' => 's');
    $params[] = array('value' => $jsonData->wineYear, 'type' => 's');
    $params[] = array('value' => $jsonData->wineType, 'type' => 's');
    ...
    $bindParams = array_merge(array($statement, $paramTypes), $paramValues);
